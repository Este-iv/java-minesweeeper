...
public class GridUtil
{
	...
	for( int i = 0 ;i< grid.length; i ++){
		for(int j = 0; j< grid[0].length; j++){
			...
			if(c.isMine()){
				...
				c.setCount(GridUtil.countNeighboringMines(grid, i, j));
			}
		}	
	}
  }
  ...
  {	
	  for(int i=0; i<grid.length; i++){
		for(int j=0;j<grid[0].length;j++){
			Cell c = grid[i][j];
			if(c.isMine() == false && c.getStatus()== Status.REVEALED)
			{
				return true; 
			}
  ...
  public static Cell findOneHiddenNeighbor(Cell[][] grid, int givenRow, int givenCol)
  {
	    ...
	    for (int row = upper; row <= lower; row += 1)
	    {
	      for (int col = left; col <= right; col += 1)
	      {
	        ...
	        {
	          Cell e = grid[row][col];
	          if(e.getStatus() == Status.HIDDEN && e.getCount() > 0 )
	          {
	        	  return e;
	          }
	        }
